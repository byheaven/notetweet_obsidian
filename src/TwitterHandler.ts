...
export class TwitterHandler {
  ...
  public async connectToAccount(account: TwitterAccount, skipVerification: boolean = false): Promise<boolean> {
    try {
      ...
      // Store the connection without verification (to avoid Buffer polyfill issues)
      ...
      // Only verify if explicitly requested (e.g., from Test Connection button)
      if (!skipVerification) {
        try {
          console.log("[NoteTweet] Verifying credentials for account:", account.name);
          await client.v2.me();
          ...
          console.error(`Twitter verification failed for ${account.name}:`, e);
          // Don't fail the connection, just update status
          account.connectionStatus = 'failed';
          account.lastConnectionTest = Date.now();
          account.lastError = this.extractErrorMessage(e);
          await this.plugin.saveSettings();
          // Still return true - client is created, posting might work
        }
      }
  ...
  public async connectToAccountById(accountId: string, skipVerification: boolean = true): Promise<boolean> {
    const account = this.plugin.getAccountById(accountId);
    if (!account) {
      return false;
    }

    return await this.connectToAccount(account, skipVerification);
  }
  ...
  public async postThread(threadContent: string[], accountId?: string) {
    ...
    try {
      ...
      // Use native tweetThread method (restored from upstream)
