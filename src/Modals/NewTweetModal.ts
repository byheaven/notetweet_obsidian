...
export class NewTweetModal extends PostTweetModal<IScheduledTweet | ITweet> {
  ...
  ): Promise<ITweet | IScheduledTweet> {
    // Get plugin instance
  ...
  private postTweets() {
    return async () => {
      ...
      if (!this.selectedAccountId) {
        log.logError("No account selected for posting");
        return;
      }

      const tweet: ITweet = new Tweet(threadContent, this.selectedAccountId);
      this.resolve(tweet);
      this.close();
    };
  }
  ...
  scheduleTweets() {
    return async () => {
      ...
      if (!this.selectedAccountId) {
        log.logError("No account selected for scheduling");
        return;
      }

      const scheduledDateTime: number = await promptForDateTime(this.app);
      const tweet: IScheduledTweet = new ScheduledTweet(
        threadContent,
        scheduledDateTime,
