...
export class ScheduledTweetsModal extends Modal {
  ...
  private async display(): Promise<void> {
    ...
    const heading = this.contentEl.createEl("h2", {
      text: `Scheduled tweets (${scheduledTweets?.length ?? 0})`,
    });
    heading.style.marginBottom = "0";

    if (scheduledTweets.length === 0) {
      this.contentEl.createEl("p", {
        text: "No scheduled tweets. Go write some! 😁",
      });
    } else {
      const scheduledTweetsContainer: HTMLDivElement = this.contentEl.createDiv(
  ...
  private addTweetRow(tweet: IScheduledTweet, container: HTMLDivElement): void {
    ...
    tweet.content.forEach((item, i) => {
      const tweetContainer: HTMLDivElement = rowContainer.createDiv(
        "tweetContainer"
      );
    ...
    tweetPostAt.textContent = `Scheduled for: ${window
      .moment(tweet.postat)
    ...
    const updateScheduledTweetButtonsContainer: HTMLDivElement = buttonRowContainer.createDiv(
      "updateScheduledTweetButtonsContainer"
    );
    const updateScheduledTimeButton: ButtonComponent = new ButtonComponent(
      updateScheduledTweetButtonsContainer
    );
    updateScheduledTimeButton
      .setCta()
      .setButtonText("Update scheduled time")
      .onClick(async () => {
    ...
    const editTweetButton: ButtonComponent = new ButtonComponent(
      updateScheduledTweetButtonsContainer
    );
    editTweetButton
      .setCta()
      .setButtonText("Edit")
      .onClick(async () => {
        const updatedTweet: IScheduledTweet = await UpdateScheduledTweetModal.Update(
          this.app,
          tweet
        );
